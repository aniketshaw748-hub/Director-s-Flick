 ...
 * spends credits, requires `higgsfield auth login`), 'fal' (fal.ai fallback,
 * video-only, requires FAL_KEY), or 'replicate' (Replicate fallback, video-only,
...
export { ReplicateProvider, ReplicateError, ReplicateAuthError } from './replicate.js';
export type { ReplicateProviderOptions } from './replicate.js';
...
export function createProvider(config: PipelineConfig, account?: ProviderAccountOpts): GenProvider {
  switch (config.provider) {
    ...
    case 'replicate':
      // Replicate fallback (video-only; images stay on Higgsfield). Token from env.
