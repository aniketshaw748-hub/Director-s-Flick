...
| T-04 | Sonnet | in-progress (sonnet) | **Review-gate backend**: REST+WS endpoints for approve / edit(instructions) / redo per shot; wire queue.ts review-ahead buffer (non-auto-approve mode); WS events for IMAGE_READY/VIDEO_READY/PLACED; EDL + redo-animation endpoint (same start_image + new prompt). | `app/src/server.ts`, `app/src/queue.ts` | Mock-provider e2e works WITHOUT `--auto-approve` by driving approvals via the API; events observable over WS |
...
| T-09 | Sonnet | in-progress (sonnet) | **Review fixes**: apply confirmed T-01 findings. | `app/src/**` | Each finding fixed or explicitly waived with reason; typecheck + mock e2e still green |
