...
| T-17 | AGV | open | **T-11 fixes (HIGH)**: implement the `@`-mention autocomplete in ReviewPage + MobileReviewPage Edit/Redo dialogs (port `.autocomplete-popover` from `design/desktop-review.html:260-270`; element list from project state); fix `MobileReviewPage.tsx:88` token bypass (`var(--text-1)`, `var(--r-sm)`); add `// TODO(T-04)` comments on the dead `shot_updated` handler + unwired redo-animation button (keep them — they go live with T-04). | `ui/src/**` | Autocomplete works against project elements; tsc clean; tokens used |
| T-18 | AGV | open | **T-11 fix (MED) — typed API layer**: replace `any` shot/project types in `ui/` with a type-only import from `app/src/types.ts` if the Vite/tsc toolchain allows; otherwise create `ui/src/apiTypes.ts` mirroring ONLY the needed interfaces with a "source of truth: app/src/types.ts — keep in sync" header. | `ui/src/**` | No `any` for shots/projects; tsc clean |
