...
3. Current queue: (1) finish the **v0.1.2 cut** (tag note -> Fable pushes); (2) **T-87** (add-account UI in App.tsx AccountChip — reassigned from Fable-2 who is on owner-directed responsive work; spec in board row; coordinate App.tsx via board note if needed).
